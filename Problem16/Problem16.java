...
static Map<String, Float> check = new HashMap<String, Float>();
  public static class TokenizerMapper1
	...
	float enrollments=(float)Integer.parseInt(itr1[7]);
	float capacity=(float)Integer.parseInt(itr1[8]);
	if(check.containsKey(itr1[6])){
		check.put(itr1[6],check.get(itr1[6])+(float)1);
	}
	else{
		check.put(itr1[6],(float)1);
}
...
Configuration conf = new Configuration();
    ...
    job.setJarByClass(Problem16.class);
